...
export async function GetTopTenAbsent() {
  try {
    ...
    const topTen = await axios.get(
      `${process.env.MAIN_SERVER_URL}/user/school/attendance/top-10-absent`,
      {
        params: {
          page: 1,
        },
...
export async function GetAllAttendanceTeacher({ teacherId }) {
  try {
    ...
    const attendances = await axios.get(
      `${process.env.MAIN_SERVER_URL}/user/school/attendance/teacher/get-all-attendance`,
      {
        params: {
          teacherId: teacherId,
        },
...
export async function GetTopTenSick() {
  try {
    ...
    const access_token = cookies.access_token;
    const topTen = await axios.get(
      `${process.env.MAIN_SERVER_URL}/user/school/attendance/top-10-sick`,
      {
        params: {
          page: 1,
        },
...
export async function GetTopTenHoliday() {
  try {
    ...
    const access_token = cookies.access_token;
    const topTen = await axios.get(
      `${process.env.MAIN_SERVER_URL}/user/school/attendance/top-10-holiday`,
      {
        params: {
          page: 1,
        },
...
export async function GetAttendanceClassroom({ classroomId, teacherId }) {
  try {
    ...
    const attendance = await axios.get(
      `${process.env.MAIN_SERVER_URL}/user/school/attendance/get-attendance-classroom`,
      {
        params: {
